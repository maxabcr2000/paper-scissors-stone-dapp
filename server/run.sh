...
# Start the Cartesi HTTP-Dispatcher and the paper-scissors-stone-dapp.
# This script must run inside the cartesi machine

DAPP_PORT=5003
HTTP_DISPATCHER_PORT=5004

# Change dir to paper-scissors-stone-dapp root
...
do
    ...
    if [ "$RETRY" == "100" ]
    then
        echo "paper-scissors-stone dapp timed out"
        return 1
    fi
done
echo ""
